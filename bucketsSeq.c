...
int bucketSort(int* array,int* arrayOut, int tamArray,int nBuckets){
    int i,j;
...
int main(int argc,char *argv[]){ 
    int tamArray=atoi(argv[1]);			//Size of the array we want to sort
    int nBuckets=atoi(argv[2]);			//Size of buckets
    int *array =malloc(tamArray * sizeof(int)); 	//Allocate space to the array
    int *arrayOut=malloc(tamArray * sizeof(int));
    
    printf("tamArray=%d,nBuckets=%d\n",tamArray,nBuckets);
	...
	bucketSort(array,arrayOut,tamArray,nBuckets);
